...
class Generation:
    def __init__(self, target_model, target_seed, target_trunc):
        ...
        if not self.file_exists(f'.\stylegan2_models\{self.model}'):
    ...
    def generate(self):
        ...
        os.system(f'python .\stylegan2\generate.py --outdir=.\out\{random_dir_name} --trunc={self.target_trunc} --seeds={self.target_seed} --network=.\stylegan2_models\{self.model}')
        #Wait for image to be generated
        while not os.path.exists(f'.\out\{random_dir_name}'):
            pass
        for image_name in os.listdir(f'.\out\{random_dir_name}'):
            #Open as PIL image
            image = Image.open(f'.\out\{random_dir_name}\{image_name}')
            return image
    ...
    def download_model(self, name):
        print(f'https://api.ngc.nvidia.com/v2/models/nvidia/research/stylegan2/versions/1/files/{name}')
        wget.download(f'https://api.ngc.nvidia.com/v2/models/nvidia/research/stylegan2/versions/1/files/{name}', f'.\stylegan2_models\{name}')
